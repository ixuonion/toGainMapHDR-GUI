// Frozen pre-refactor implementation, used only for comparative benchmarks.
import Foundation
@testable import GainMapHDRApp

enum LegacyServiceError: LocalizedError {
    case failed(Int32)
...
protocol LegacyBackendConverting: Sendable {
    func run(command: ConversionCommand, progress: @escaping @Sendable (String) -> Void) async throws
    func cancel()
}

final class LegacyBackendProcessService: LegacyBackendConverting, @unchecked Sendable {
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            process.terminationHandler = { [weak self] process in
                ...
                if process.terminationStatus == 0 {
                    ...
                    continuation.resume(throwing: LegacyServiceError.failed(process.terminationStatus))
                }
            }
